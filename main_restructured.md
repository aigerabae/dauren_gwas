# 0. Software Versions & Environment
...
# 1. Genotype Calling (IDAT → GTC → VCF)

## 1.1 Array Genotype Calling & GTC-to-VCF Conversion
...
# 2. Per-Sample VCF QC (Pre-Merge)

## 2.1 REF/ALT Consistency Check
...
# 4. Post-Merge Variant Processing & Annotation

## 4.1 Merged VCF Summary Statistics (bcftools stats)
...
## 4.2 REF/ALT Consistency Check (Merged File)
...
## 4.3 Realignment & Multiallelic Splitting
...
## 4.7 QC Filtering Summary & Notes
...
# 5. PLINK Conversion & Sample-Level QC

## 5.1 VCF to PLINK Conversion & Duplicate Variant Check
...
*Author's original label: PLINK QC*
*Author's original label: Setup and initial conversion*
```
...
*Author's original label: Duplicate variant check (sanity check post-VCF-conversion)*
```
# if nonzero, remove
...
## 5.2 Call Rate Filtering & Heterozygosity Outliers

*Author's original label: Sample-level QC — call rate, heterozygosity, sex*
```
...
# 6. Relatedness & Variant-Level QC

## 6.1 Relatedness (KING)

*Author's original label: Relatedness — KING instead of PI_HAT*
```
...
## 6.2 Differential Missingness, HWE & MAF Filtering

*Author's original label: Variant-level QC — call rate, HWE, MAF*
```
...
## 7.1 LD Pruning, Internal PCA & Scree Plot

*Author's original label: LD pruning + internal PCA + scree plot* 
```
...
## 7.2 Ancestry Outlier Flagging — Round 1 (Internal PCA)

*Author's original label: Saving ancestry outliers according to the internal PCA*
```python
...
## 7.3 PCA with Reference Populations

*Author's original label: PCA with other referent populations to see any outliers*
```
...
## 7.4 PC1 vs PC2 Visualization & Iterative Ancestry Outlier Removal (Rounds 1–3)

*Author's original label: PC1 vs PC2 for case and control (before removing outliers)* 
...
## 8.1 Primary Model — PCA-Adjusted Firth Logistic Regression

*Author's original label: Association testing — PCA-adjusted logistic regression, not --model*
All 10 PCs:
```
...
# 9. Genomic Inflation & Visualization

## 9.1 Lambda GC, QQ Plot & Manhattan Plot

*Author's original label: Compute λ_GC and generate QQ/Manhattan plots on this corrected file*
```
...
# 10. Multiple Testing Correction (Part 1)

## 10.1 Naive Bonferroni on LD-Pruned Variant Set (d13_pruned)

*Author's original label: calculating effective number of tests*
```bash
...
# 11. Allele Frequency Sanity Check

*Author's original label: MAF table sanity check*
```
# for all
...
*Author's original label: formal power calculation*
```R
...
## 12.2 Power at Suggestive Significance (α = 1e-5)
...
# 13. Multiple Testing Correction (Part 2) — Revised Pruning & simpleM

## 13.1 Suggestive-Threshold Definition & Rationale
...
## 13.2 Revised LD-Pruned Bonferroni (Second Pass)
...
## 13.3 simpleM — Effective Number of Independent Tests (Autosomes)
...
# 14. Sensitivity Analyses — Alternative Models

## 14.1 Sex-Unadjusted Model, Full Firth vs. Firth-Fallback, Permutation & Genotypic Model

*Author's original label: I will try other models*
1. Sex not adjusted:
```
...
# 15. Functional Annotation of Top Hits

*Author's original label: Annotation*
```
...
# 16. Polygenic Risk Score (PRS)

## 16.1 Asia-Base PRS — Liftover, Reconciliation, Clumping & Scoring

*Author's original label: PRS*
```
...
## 16.2 Europe-Base PRS — Clumping & Scoring
...
## 16.3 Asia vs. Europe PRS Comparison & Summary
...
# 17. Gene-Based Testing (MAGMA)

## 17.1 Reference Panel Prep, Annotation & Gene-Based Association Test

*Author's original label: MAGMA*
```
...
# TBC — Notes on This Restructuring

No subsection in the document above is genuinely empty (i.e. a header with zero content
before the next header) once the duplicate-numbering problem below was fixed. This section
exists as a transparency log rather than a bucket of unfinished stubs.

**What was demoted, and why:** the original file mixed two independent header systems —
the author's own ad-hoc inline labels (lines starting with a single `#`, e.g. `# 9. Association
testing...`, `# 11 calculating effective number of tests`) and the numbered `#`/`##` section
headers added for navigation. Because both used the same Markdown level, they rendered as
competing top-level headers with mismatched, sometimes duplicate, numbers (e.g. two things
labeled "11"), and made downstream headers appear to jump backward (e.g. `7.2` immediately
followed by an unrelated `# 6`). Every one of the author's original inline labels was kept —
none were deleted — but converted from a Markdown header into italicized inline text
(`*Author's original label: ...*`) so there is only one numbering system in the document.
These are listed here for cross-reference against your original notes:

- PLINK QC:
- Setup and initial conversion
- Duplicate variant check (sanity check post-VCF-conversion)
- Sample-level QC — call rate, heterozygosity, sex
- Relatedness — KING instead of PI_HAT
- Variant-level QC — call rate, HWE, MAF
- LD pruning + internal PCA + scree plot
- Saving ancestry outliers according to the internal PCA
- PCA with other referent populations to see any outliers
- PC1 vs PC2 for case and control (before removing outliers)
- Association testing — PCA-adjusted logistic regression, not --model
- Compute λ_GC and generate QQ/Manhattan plots on this corrected file
- calculating effective number of tests
- MAF table sanity check
...
**Other structural fixes made:**
- Section 10 vs. 13 (Multiple Testing Correction): the document contains two separate,
  non-adjacent rounds of effective-test/threshold work. Rather than force them under one
  "10" (which would require jumping back to an earlier number after Sections 11–12), the
  second round is now its own Section 13, placed exactly where it physically occurs.
- `## 12.2 Power at Suggestive Significance` was moved to sit directly above the suggestive-
  power R script (it previously sat above an unrelated reference-quote block further down,
  leaving the actual suggestive-power code mislabeled under `12.1`).
- `## 17.1`/`17.2` (MAGMA) were retitled to match what's actually under them: `17.1` now
  covers reference-panel prep *and* the gene-based test run itself (both were originally
  lumped together); `17.2` is specifically the SNP-to-gene cross-referencing step.
- No code, commands, or interpretive text were moved, reordered, or edited anywhere in this
  pass — only header placement, header titles, and the formatting of the author's own inline
  labels changed.
